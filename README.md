...
If you decided to run anubis, you must be aware that because of how it acts as "middle ware" in NGIX, client IP, Port and other original Client Information are going to be sent 
via headers from NGINX to PHP on the backend. By default Drupal and Symfony will not trust those headers (nor its origin) and that will intefere with IP Based embargo Bypass Logic as defined on the Format Strawberry Field Module (if you set that up). If you don't act on this, still any IP based embargo ADOs will be secure. So *secure* literally nobody other than a logged in user will be able to access them because the Client IP that PHP will see is the one of the NGINX Container (inside Docker).

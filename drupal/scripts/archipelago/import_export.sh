...
Help()
{
   ...
   echo
   echo "Syntax: ./import_export.sh [-h|n|i|e|j|d|s]"
   echo "options:"
   ...
   echo "Examples:"
   echo
   echo "./import_export.sh -e -j /home/user/archipelago-deployment-live/deploy/ec2-docker/.env -d https://archipelago.nyc -s /home/user/archipelago-deployment-live/drupal/d8content/export"
   echo
   echo "./import_export.sh -i -j /home/user/archipelago-deployment-live/deploy/ec2-docker/.env -d https://archipelago.nyc -s /home/user/archipelago-deployment-live/drupal/d8content/import"
   echo
}
...
domain_questions() {
  ...
  if [ "$archipelago_domain_set" = false ]
  then
...
interactive_prompts() {
  ...
  elif [ "${selected_metadata_action}" == "import" ]
  then
    source_target=""
...
while getopts ":hniej:d:s:" option; do
  case $option in
    h) # Display Help
       Help
       exit;;
